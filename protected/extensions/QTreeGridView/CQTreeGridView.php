                    ...
                    if($model->$level==1) { //root with level=1
                        ...
                        if($model->$level == $currentLevel) {
                            if(is_null($stack[count($stack)-1])) {
                                throw new Exception(Yii::t('yiiext','Tree is corrupted'));
                            }
                ...
                $("#'.$this->getId().' .items tr.initialized, #'.$this->getId().' .items tr.before, #'.$this->getId().' .items tr.after").droppable({
                    ...
                    drop: function(e, ui) {
                      ...
                      if($(this).hasClass("initialized")) {
                        window.location.href = "/admin/catalog/default/moveNode/pid/'.$this->filter->root.'/action/child/to/"+$(this).attr("id")+"/id/"+$(ui.draggable).attr("id");
                      }
                      if($(this).hasClass("before")) {
                        window.location.href = "/admin/catalog/default/moveNode/pid/'.$this->filter->root.'/action/before/to/"+$(this).attr("id").replace("before-", "")+"/id/"+$(ui.draggable).attr("id");
                      }
                      if($(this).hasClass("after")) {
                        window.location.href = "/admin/catalog/default/moveNode/pid/'.$this->filter->root.'/action/before/to/"+$(this).attr("id").replace("after-", "")+"/id/"+$(ui.draggable).attr("id");
                      }
                    },
                    hoverClass: "accept",
                    over: function(e, ui) {
